...
This bot is also a minor modification of [Bluesky-Canes-Bot](https://github.com/acarters/Bluesky-Canes-Bot) by [ACarters](https://github.com/acarters)! 99.9% of the credit for this bot should go to him!
...
### Reposting
This bot uses the Mastodon Canes API to collect tweets created by the Oilers. The Mastodon API returns content values in HTML, so considerable regex formatting is needed in order to get the content into a plaintext value that can be used by the Bluesky API. This Mastodon API is flawed, leading some posts to not be collected and given to the bot. This is not really a workable problem, as the code sees the Mastodon API as a black box, and I am essentially forced to play telephone with the posts through a middleman unless I am willing to spend the money to pay Elon Musk for Twitter API. I cannot guarantee that this bot will repost every post made by the Oilers on Twitter. Instead, I guarantee that this bot will repost all posts collected by the Mastodon API.
...
Giveaways performed by the Oilers admin are not valid unless there is an external link to the giveaway. The bot detects common keywords related to giveaways from the admin, and adds a disclaimer saying that the giveaway is not valid on bluesky.

## Things To Do
Although this bot is currently up and running on Bluesky, this codebase is still under development. As such, there are changes left to add. Some are listed below: (NOTE: I am not experienced enough with programming yet, so these features will only be added if the original repo adds them)
